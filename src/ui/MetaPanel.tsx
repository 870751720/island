...
export function MetaPanel({ onClose, onLearn }: { onClose: () => void; onLearn: () => void }) {
  ...
  const previewLevel = preview ?? Math.min(level, META_MAX_LEVEL - 1);
                ...
                {entry.nodes.map((talent) => {
                  ...
                  return <div className={`meta-path${rank > 0 ? ' learned' : ''}`} key={talent.id}>
                    <button className={`meta-talent${selected === talent.id ? ' selected' : ''}${!open ? ' locked' : ''}${rank > 0 ? ' learned' : ''}`}
                      ...
                      onClick={() => { setSelected(talent.id); setPreview(null); setNotice('点击天赋查看效果，再消耗心得领悟'); }}>
