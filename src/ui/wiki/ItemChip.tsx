...
/** 图鉴中可跳转的材料、食物与产物。 */
export function ItemChip({ kind, count, note, onOpen }: { kind: ResourceKind; count?: number; note?: string; onOpen: (kind: ResourceKind) => void }) {
  const tap = useScrollAreaTap(() => onOpen(kind));
  return (
    <button className={`${styles.chip}${note ? ` ${styles.chipWithNote}` : ''}`} {...tap} aria-label={`查看${ITEMS[kind].name}${note ? `，${note}` : ''}`}>
