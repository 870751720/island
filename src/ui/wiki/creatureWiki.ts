...
import { HEART_MAX, HEART_DURATION, HOME_RADIUS, FOOD_SEARCH_RADIUS, FORAGE_THRESHOLD, STORED_FOOD_THRESHOLD, LIVESTOCK_PRODUCE, PRODUCTION_SECONDS, type TameSpecies } from '@/game/systems/AnimalHusbandry';
  ...
  return {
    ...
    interaction: tame
      ? ['用', { kind: 'lasso', label: '套索' }, isLassoPredator(id) ? '抓住后，必须等待五次挣脱判定全部失败，才能开始投喂；驯养成功前仍会攻击。' : '抓住后即可开始投喂。', '主动丢下适合的食物，落地至少四秒后供它进食，也可从食料桶取食，实际爱心达到上限即可驯养（成功前填充最多显示九成）。驯养后全队共享；爱心归零会恢复野生。',
        `解绳后在活动中心半径 ${HOME_RADIUS} 米内活动。首次驯养及日常觅食只搜索自身周围 ${FOOD_SEARCH_RADIUS} 米内的适配食物，食物还须满足绳长、家园边界和围栏通行限制。满心无食物可维持 ${HEART_DURATION[id as TameSpecies] / 60} 分钟；低于 ${FORAGE_THRESHOLD * 100}% 优先寻找自然食物，低于 ${STORED_FOOD_THRESHOLD * 100}% 才自动取食桶内或地上食物，每口恢复达到 ${STORED_FOOD_THRESHOLD * 100}% 后停止取用储粮。`,
        id === 'wolf' ? `狼只吃适配鱼肉，不吃植物；日常一份烤兽肉约抵消 ${Math.round(animalFoodHeart('cookedGameMeat', id, true) / HEART_MAX[id] * HEART_DURATION[id] / 60)} 分钟消耗。`
          ...
          : `每只${ANIMAL_LABELS[id]}配 ${id === 'rabbit' ? 1 : id === 'sheep' ? 2 : 3} 个处于觅食范围内、可到达且未被采集或争食的草丛，可长期维持驯养，不必额外投粮。多只动物按数量累加。`]
