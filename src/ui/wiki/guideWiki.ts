...
export const GUIDE_ENTRIES: readonly GuideEntry[] = [
  ...(process.env.NEXT_PUBLIC_XHS_EXPORT === '1' ? [] : [{
    ...
    sections: [
      { title: '选择连接方式', text: ['创建或加入房间时，可以选择好友直连或服务器中转。好友直连是默认方式，和星露谷的直连联机类似：双方网络需要可以直接互通，如果和朋友不在同一局域网内，很可能连不上，连不上时改用服务器中转。手动输入房间码时，需与房主选择相同的连接方式；邀请链接和二维码会自动选择。单机游戏也可以在设置中选择方式后开启多人模式。'] },
