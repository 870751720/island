...
export const GUIDE_ENTRIES: readonly GuideEntry[] = [
  ...(process.env.NEXT_PUBLIC_XHS_EXPORT === '1' ? [] : [{
    id: 'multiplayer', title: '与朋友联机', summary: '选择连接方式，分享房间码，一起建设房主的岛。',
    sections: [
      { title: '选择连接方式', text: ['创建或加入房间时，可以选择好友直连或服务器中转。好友直连是默认方式；直连困难时可尝试服务器中转。局域网暂未开放。手动输入房间码时，需与房主选择相同的连接方式；邀请链接和二维码会自动选择。单机游戏也可以在设置中选择方式后开启多人模式。'] },
      { title: '中转名额', text: ['服务器中转有房间数和每房人数限制，以联机界面显示为准，人数包含房主。开房名额满时不能新建中转房间，但已有房间仍可加入空位；房间满员时需等待其他玩家离开。好友直连不占中转名额。'] },
      { title: '岛屿与断线', text: ['岛屿始终由房主运行与保存，中转不会代替房主继续游戏。房主退出后房间解散；短暂切后台容忍45秒无消息；客人断线后回到前台会自动尝试恢复，也可返回加入页。房主仍在线且房间有空位时，可在五分钟内用原房间码与相同连接方式重新加入恢复角色。角色恢复记录不保留在线人数名额。中转房主断线后可保留岛屿，在设置中重新开房并分享新邀请。'] },
    ],
  }]),
  ...(process.env.NEXT_PUBLIC_XHS_EXPORT === '1' ? [] : [{
    id: 'cloud-save', title: '存档与找回', summary: '记住存档码，手动备份与找回全部游戏数据。',
    sections: [
      { title: '记住存档码', text: ['非小红书版本开始游戏时，如果本地没有存档码，会提示输入最多 20 位存档码，区分大小写。可以跳过，但本地数据丢失后可能无法找回。H5 本地数据可能丢失，一定要记住存档码；知道同一码的人可以读取和覆盖同一份云档，请不要告诉别人。本机记住后无需重复输入，也可以更换存档码。'] },
      { title: '手动备份', text: ['首次输入后先检查云档：已有云档可选择云端或本地版本；选择云端只覆盖本地，不上传。选择本地或新码同意备份后，进入游戏上传一次。本地仍然自动保存，之后云端需你手动上传更新。每个存档码只有一份云档，每次上传覆盖上一次备份，包含岛屿、传承、设置和联机记录等全部游戏本地数据。小红书版本不提供云存档。'] },
      { title: '找回进度', text: ['输入原存档码，下载后核对保存时间与天数，再确认覆盖本机数据。死亡后也可以读回先前备份，或上传死亡后的数据。没有岛屿的备份会清除本机岛屿，确认前请留意提示。房主离线的联机房间不会因下载存档而恢复。'] },
    ],
  ...
  {
    id: 'cooking', title: '烹饪与饮水', summary: '烧烤、烹饪与料理研究，安排稳定的食物和水源。',
    sections: [
      { title: '烤制食物', text: ['放下', item('campfire', '火堆'), '，保持燃烧后选择食材和份数烤制。烤制时保持站定，走开会中断；留出背包空间收取熟食。'] },
      { title: '升级厨房', text: ['用', item('shovel', '铲'), '回收燃尽的火堆，得到', item('deadCampfire', '熄灭的火堆'), '，再按配方制作', item('cookingStation', '烹饪台'), '。它既能烧烤，也能烹饪汤品和已发现的面点；下锅后自动加工，燃尽会暂停，添柴可继续，做好后记得收取。'] },
      { title: '发现新料理', text: ['四级工作台可制作', item('researchTable', '料理研究台'), '。按线索投入一到四种食材，每种一份，顺序不限；研究两秒，正确组合一定成功，获得一份成品并解锁食谱。失败消耗材料并等待五秒。研究或获得成品的发现记录跨存档保留，未发现的料理在图鉴中显示待发现；已发现页可查看搭配和烹饪用量。'] },
  ...
  },
  {
    ...
    sections: [
      ...
      { title: '持续照料', text: ['驯养阶段和成功后都可以用', item('feedBarrel', '食料桶'), '供食。首次驯养和日常觅食都只搜索动物自身周围15米内的食物。成功后解绳可在中心半径30米内活动，优先吃自然植物，低于50%爱心才吃储粮。每只兔子配1个草丛、羊配2个、野牛配3个，在15米觅食范围内、可到达且无人采集或争食时可长期自给。爱心会逐渐减少，归零后恢复野生；', creature('wolf', '狼'), '和', creature('bear', '熊'), '会重新具有攻击性；狼熊满心无食物可维持60分钟，日常鱼肉恢复量提高。这两种猛兽首次捕捉还需等待挣脱判定结束，驯养成功前仍然危险。'] },
  ...
  },
  {
    ...
    sections: [
      ...
      { title: '联机分工', text: ['可以分工采集、种植和加工，共用岛上的设施。自己的移动、采集动作和进度即时响应；网络较慢时，物品到账和共享设施变化会稍后确认。同时采集同一资源时，以实际结算结果为准。制作时先把需要的材料取到自己的背包；查看指南与图鉴不会替队友操作，联机阅读也不会暂停整个岛屿。'] },
    ],
  },
];
