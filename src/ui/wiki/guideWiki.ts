...
export const GUIDE_ENTRIES: readonly GuideEntry[] = [
  ...(process.env.NEXT_PUBLIC_XHS_EXPORT === '1' ? [] : [{
    ...
    sections: [
      { title: '记住存档码', text: ['开始主界面的云存档首次使用需要输入最多 20 位存档码，区分大小写。H5 本地数据可能丢失，一定要记住存档码；知道同一码的人可以读取和覆盖同一份云档，请不要告诉别人。本机记住后无需重复输入，也可以更换存档码。'] },
      { title: '手动备份', text: ['本地仍然自动保存，云端只在你确认上传时更新。每个存档码只有一份云档，每次上传覆盖上一次备份，包含岛屿、传承、设置和联机记录等全部游戏本地数据。小红书版本不提供云存档。'] },
