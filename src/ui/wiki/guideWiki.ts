...
export const GUIDE_ENTRIES: readonly GuideEntry[] = [
  ...(process.env.NEXT_PUBLIC_XHS_EXPORT === '1' ? [] : [{
    ...
    sections: [
      { title: 'UU 辅助直连（实验性）', text: ['双方先加入同一个 UU 云联机房间，在好友直连的「UU 虚拟局域网」中各填本机的虚拟 IPv4，然后创建或加入六位码房间。不要填朋友的 IP；留空即普通直连。地址仅本页临时使用，刷新需重填，修改后需重新连接。功能仅补充连接路径，不保证浏览器可使用 UU。展开连接诊断可查看握手状态与选中地址，连接失败时可复制双方诊断供排查。'] },
      { title: '选择连接方式', text: ['创建房间时，可以选择好友直连或服务器中转。好友直连是默认方式，需要双方网络可以直接互通；连接失败时，请确认房主在线，并检查双方网络后重试。加入时输入房间码即可自动识别：6 位为好友直连，5 位为服务器中转；邀请链接和二维码同样按房间码识别。当前环境不支持好友直连时，可复制网页版链接到手机系统浏览器打开，不会自动改用中转。单机游戏可在设置中点击「开启多人模式」，在弹窗选择方式并确认创建，成功后显示房间码。'] },
      { title: '中转名额', text: ['服务器中转只有少量免费房间，房间数和每房人数有限制，以联机界面显示为准，人数包含房主。开房名额满时不能新建中转房间，但已有房间仍可加入空位；房间满员时需排队等待其他玩家离开。好友直连不占中转名额。'] },
