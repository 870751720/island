...
const BOTTLES = [
  { label: '生命', warning: '危险', color: '#e94659', path: 'M17 10H27V17C43 26 38 45 22 45S1 26 17 17Z' },
  { label: '饱食', warning: '饥饿', color: '#eaaa29', path: 'M17 10H27V17Q36 19 36 27V37Q36 45 28 45H16Q8 45 8 37V27Q8 19 17 17Z' },
  { label: '水分', warning: '缺水', color: '#35b5dd', path: 'M17 10H27V17Q33 22 33 28V36Q33 45 22 45Q11 45 11 36V28Q11 22 17 17Z' },
] as const;
        ...
        return <div className={`hud-bottle${low ? ' is-low' : ''}`} key={bottle.label}>
          <div className="hud-bottle-meter" role="meter" aria-label={bottle.label} aria-valuemin={0} aria-valuemax={100} aria-valuenow={Math.round(level * 100)} aria-valuetext={`${Math.round(level * 100)}%${low ? `，${bottle.warning}` : ''}`}>
            <svg className="hud-bottle-fallback" viewBox="0 0 44 48" aria-hidden="true">
              <defs><clipPath id={`${id}-${index}`}><path d={bottle.path} /></clipPath></defs>
              <path d={bottle.path} fill="#f4fae977" />
              <g clipPath={`url(#${id}-${index})`}><rect x="6" y={45 - level * 33} width="32" height={level * 33} fill={bottle.color} /></g>
              <path d={bottle.path} fill="none" stroke="#749790" strokeWidth="1.5" />
