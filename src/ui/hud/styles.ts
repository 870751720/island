...
${hudControlStyles}
.hud-top-edge{top:calc(max(7px,var(--game-safe-top)) + min(var(--hud-top-offset,0px),calc(15 * var(--game-vh))))}
.hud-status{position:absolute;container:hud-status / inline-size;left:max(8px,var(--game-safe-left));display:flex;flex-direction:row;flex-wrap:wrap;align-items:flex-start;gap:4px;right:calc(max(10px,var(--game-safe-right)) + var(--hud-right-reserve));pointer-events:none;box-sizing:border-box;font-family:Arial,"PingFang SC","Microsoft YaHei",sans-serif;color:#49665e}
...
@container game-viewport (orientation:landscape) and (max-height:500px){.quest-card{padding:5px 7px;gap:2px}.quest-hint{display:none}}
...
.hud-buffs{display:flex;flex-wrap:wrap;gap:4px;flex:1;min-width:26.4px;max-height:calc(40 * var(--game-vh));overflow-y:auto;scrollbar-width:none;pointer-events:none;overscroll-behavior:contain}
...
@container game-viewport (orientation:landscape) and (max-height:500px){.hud-backpack,.hud-tool{top:auto;bottom:max(30px,var(--game-safe-bottom))}.hud-backpack{right:calc(max(16px,var(--game-safe-right)) + 86px)}}
