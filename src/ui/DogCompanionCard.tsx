...
export function DogCompanionCard({ dog }: { dog: HudSnapshot['dog'] }) {
  ...
  return <section aria-label="薯条" style={{ marginTop: 10, padding: '12px 10px', borderRadius: 12, background: gameTheme.inset, color: gameTheme.ink }}>
    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      <div style={{ minWidth: 0 }}>
        <div style={{ fontWeight: 700, fontSize: 14 }}>薯条 <span style={{ fontWeight: 400, fontSize: 11, color: gameTheme.muted }}>同行的小伙伴</span></div>
    ...
    <div role="progressbar" aria-label="薯条成长" aria-valuemin={0} aria-valuemax={100} aria-valuenow={Math.round(progress * 100)} aria-valuetext={`成长经验 ${dog.xp}，第 ${stage.stage} 阶段`}
