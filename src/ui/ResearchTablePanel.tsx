...
import { HIDDEN_RECIPES, RESEARCH_INGREDIENTS, matchResearch, type HiddenFood } from '@/game/systems/HiddenRecipes';
...
export function ResearchTablePanel({ hud, onStart, onClose }: { hud: HudSnapshot; onStart: (kinds: ResourceKind[]) => boolean; onClose: () => void }) {
  ...
  const [clueKind, setClueKind] = useState<HiddenFood | null>(null);
  ...
  const availableIngredients = (r: (typeof HIDDEN_RECIPES)[number]) => r.research.filter(k => count(k) > 0).length;
  const clue = unknown.find(r => r.kind === clueKind) ?? [...unknown].sort((a, b) => availableIngredients(b) - availableIngredients(a))[0];
  const clueIndex = unknown.findIndex(r => r.kind === clue?.kind);
  const changeClue = (step: number) => {
    setClueKind(unknown[(clueIndex + step + unknown.length) % unknown.length]?.kind ?? null);
    setMoreHint(false);
  };
  useEffect(() => { setMoreHint(false); }, [clue?.kind]);
  ...
  return <div className={styles.overlay} onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
    <section role="dialog" aria-modal="true" aria-label="料理研究台" style={{ ...gamePanelStyle, maxHeight: 'none', overflow: 'hidden' }} className={styles.panel}>
      ...
      {tab === 'known' ? <DiscoveredRecipes hud={hud} /> : <div className={styles.tryBody}>
        <div className={styles.preparation}>
        <div className={styles.card}>
          {clue ? <><div className={styles.header}><strong>线索 {clueIndex + 1}/{unknown.length}</strong>
            <div className={styles.clueNavigation}>
              <button style={gameButtonStyle} disabled={unknown.length < 2} onClick={() => changeClue(-1)}>上一条</button>
              <button style={gameButtonStyle} disabled={unknown.length < 2} onClick={() => changeClue(1)}>下一条</button>
            </div></div>
            <p>{moreHint ? clue.hint : clue.clue}</p>
            {research.failures >= 2 && !moreHint && <button style={gameButtonStyle} onClick={() => setMoreHint(true)}>查看进一步提示</button>}
          </> : <><strong>当前料理已全部发现</strong><p>打开「已发现」查看搭配与烹饪用量。</p></>}
        </div>
