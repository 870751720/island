...
export function NewGameDialog({ save, value, onChange, pet, onPetChange, onConfirm, onCancel }: {
  save: SaveData | null;
  pet: CompanionKind;
  onPetChange: (kind: CompanionKind) => void;
  value: GameMode;
  ...
  return <div className="abandon-mask">
    <div className="abandon-panel" role="dialog" aria-modal="true" aria-labelledby="new-island-title"
      ...
      <p className="abandon-text" aria-live="polite">{value === 'leisure'
        ...
        : '沿用原有求生规则，本局结束后可按生存天数获得荒岛传承点。'}</p>
      <CompanionSelector value={pet} onChange={onPetChange} />
