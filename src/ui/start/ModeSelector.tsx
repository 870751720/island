...
export function ModeSelector({ value, onChange, disabled = false }: {
  ...
  return <>
    ...
    <SegmentedRow ariaLabel="游戏模式" label="游戏模式" value={value} onChange={onChange} disabled={disabled}
...
const MODE_TAGS: Record<GameMode, readonly { text: string; className: string }[]> = {
  leisure: [{ text: '可复活', className: 'tag-revive' }, { text: '无传承', className: 'tag-no-legacy' }],
  survival: [{ text: '不可复活', className: 'tag-no-revive' }, { text: '有传承', className: 'tag-legacy' }],
};

const modeSelectorCss = `
.mode-tags{display:flex;gap:4px;min-width:0;flex-wrap:wrap}
.mode-tag{font-size:10px;line-height:1;padding:4px 6px;border-radius:999px}
