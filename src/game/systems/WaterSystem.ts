...
export class WaterSystem {
  ...
  update(delta: number, harvestBusy: boolean): void {
    const nearWater = this.ponds.some(
      (p) =>
        this.player.group.position.distanceTo(p.position) <
        p.radius + DRINK_RANGE
    );
