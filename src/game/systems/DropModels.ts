...
export const DROP_COLORS: Record<ResourceKind, string> = {
  ...
  cuttlefish: '#6b5f8a',
  anchovy: '#a9c3cc',
  horseMackerel: '#8ba3a0',
  yellowCroaker: '#e3c56d',
  saury: '#7d97a8',
  hairtail: '#cfd8dc',
  grouper: '#6d7b5a',
  catfish: '#5b664f',
  ...
  boiledCarrot: '#d98a4a',
  bread: '#d9a441',
  researchTable: '#b68b58', ...hiddenRecipeRecord(r => r.visual.color),
  wheat: '#d9b45a',
  potatoSeed: '#c9a06a',
...
const BUILDERS: Record<ResourceKind, () => THREE.Object3D> = {
  ...
  plankPath: () => {
    ...
    const model = new PlankPath(scene, new THREE.Vector3()).group;
    scene.remove(model);
  ...
  anchovy: () => makeFishShape(DROP_COLORS.anchovy, 0.7, 0.7, 1),
  horseMackerel: () => makeFishShape(DROP_COLORS.horseMackerel, 0.9, 0.9, 1),
  yellowCroaker: () => makeFishShape(DROP_COLORS.yellowCroaker, 0.9, 0.9, 1),
  saury: () => makeFishShape(DROP_COLORS.saury, 0.7, 0.6, 1.5),
  hairtail: () => makeFishShape(DROP_COLORS.hairtail, 0.6, 0.6, 1.7),
  grouper: () => makeFishShape(DROP_COLORS.grouper, 1.2, 1.2, 1.3),
  catfish: () => makeFishShape(DROP_COLORS.catfish, 1.1, 0.9, 1.6),
  grassCarp: () => makeFishShape(DROP_COLORS.grassCarp, 1.2, 1.1, 1.4),
  ...
  ...hiddenRecipeRecord(r => () => makeHiddenFoodModel(r.kind)),
