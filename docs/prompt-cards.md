...
| 手搓合成 | 无任一合成/烹饪进行中,且有可弹出的手搓配方(材料齐 + 未拥有 + 可见性判定)或可搭工作台;火堆卡片额外要求场上与背包里都没有火堆/烹饪台(`CampfireSystem.canStart`) |
...
- 2026-09-07:新增设置「拾取卡片放右下角」(默认关闭,`SettingsPanel` 开关行):开启后三张卡片统一改挂右下角工具按钮下方(`right: max(16px, safe-area-inset-right); top: calc(50% + 46px)`);偏好存 localStorage(`promptCardBottomRight`),纯 UI 表现,不涉及联机同步。
