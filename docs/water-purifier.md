...
- 系统 `systems/WaterPurifierSystem.ts`:对齐饵料桶 `BaitBarrelSystem` 的模式——摆放校验(`onWetBeach`:`getWaterKind === 'sea'` 或 `isNearSea(1.5)` 以内且不在水洼)、铲子站定自动挖走、`nearby` 靠近判定、`snapshot/restore/netApply` 存档与网络重放、`EntityChangeSink` 增量上报。无内部玩法状态,纯摆件。
