...
- `src/game/world/Props.ts`:`PropKind` 加 `'wormNest'`,`PROP_CONFIG` 配 240 秒再生;`makeWormNest()` 程序化建模(土丘 + 洞口 + 单只蚯蚓,蚯蚓模型单独返回用于显隐);`nestWorms` Map 管理各窝的蚯蚓模型,`syncAppearance` 按 `ready` 切换蚯蚓可见性;`placeWormNest` 支持玩家放回。
...
- 蚯蚓窝随完整资源点列表存档；`SAVE_VERSION` 32 之前的存档直接弃用，不执行旧档补撒。
