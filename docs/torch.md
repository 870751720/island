...
- 背包里点击「使用」直接插到就近最优格(详见 autoplace.md);不能在水里,格内不能被其他资源点/摆件占住。
...
- 放置、铲子挖掘回收、存档(`shrines` 快照带 kind)、联机同步(place 走 `useShrine` action、增删走 `shrines` 世界增量快照)全部沿用 `ShrineSystem`,零新增协议。
