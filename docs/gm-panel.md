...
- 入口：2 秒内点击红心 5 次 → 玩家 → 男孩整套造型。提供原版、A 涂鸦小子、B 海岛少年、C 背带裤男孩、D 野外小猎手五个触屏按钮，按钮至少 64px 高，双列布局，文字自动换行。
- 每个方案独立设计头发、五官、身材和基础服饰；全房间男孩同步切换，女孩不受影响。当前是女孩时可先用性别按钮切到男孩。
- 为完整展示方案，预览会隐藏已穿帽子、衣裤和背包的外观，装备状态和属性保留；切回原版或女孩恢复。手持工具照常显示，关闭面板后可直接对比行走和采集动作。
- 版本通过 `onSetConfig({ boyModelVariant })` 进入原 GM 同步链路，面板定时读取已应用版本以更新远端切换后的选中态。选择不写存档，刷新默认原版，性别仍按原机制持久化。
