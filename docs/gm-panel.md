...
玩家页提供「测试 5/10/20/50/100 个 Buff」与「清除测试 Buff」指令按钮。每次替换测试集合，真实 Buff 保留；测试效果含不同名称、图标、增益/减益边框及永久/固定秒数样例，点击可查看说明。关闭 GM 后检查排列、展开小地图后的换行及大量图标的触屏滚动。

测试数据只由当前 GameplayUI 持有，关闭 GM 保留，退出对局清除，不改变属性、不写存档。房主和客人可各自使用，不发送动作或同步给队友。
