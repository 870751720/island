...
- 面板按模块划分 tab:
  ...
  - 博美：阶段、成长经验、同行与护主模拟、全部冷却清除、召回、食物对照组、逐项食物领取、低血量狼实战与救场测试。
  - 特殊事件:立即触发一次喝水出鳄鱼;喝水出鳄鱼概率 %(±10 步进,保留一位小数,默认 0.5%);树生长间隔秒数(±10 步进,默认 60,每次判定有 1/2 概率升阶,作用于房主侧生长判定)。
- 性别和博美成长属于持久化进度，GM 修改也随正常存档保存；其他 GM 开关为运行时内存态,不写入存档,新对局重置为默认(允许死亡、不无敌、不锁定时刻、默认钓鱼权重、默认鳄鱼概率与生长节奏)。
...
`DogTab.tsx` 经 `Game.gmDog` 操作全队共享博美。客人上行 `gmDog(command, value)`，`ActionProtocol` 校验命令与数值范围后，由 `Actions.ts` 带操作者会话在房主结算。阶段、经验和喂食/护主奖励计时随姿态回流，UI 每 300ms 读取，不能在客人本地伪造经验。
...
博美页另有「警觉表情」「扑咬表情」「护主表情」三个预览按钮，经既有 gmDog 权威动作立即播放指定自绘图案并同步全队。切到较高阶段预览「更强了！」小字动效；关闭 GM 后打开背包角色页，核对阶段、称号与成长经验。预览不增加经验或改变战斗伤害。
