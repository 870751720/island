...
- 入口：2 秒内点击红心 5 次 → 玩家 → 男孩模型对比。提供原版、A 圆润童趣、B 自然少年、C 蓬松冒险家四个触屏按钮，按钮至少 64px 高，双列布局。
- 只影响全房间男孩的基础模型；女孩不变。当前是女孩时可先用性别按钮切到男孩。建议卸下帽子、衣服及裤子后观察脸型和四肢，关闭 GM 面板后可对比行走、采集等动作。
- 版本通过 `onSetConfig({ boyModelVariant })` 进入原 GM 同步链路，面板定时读取已应用版本以更新远端切换后的选中态。版本不写存档，页面刷新默认原版，性别仍按原机制持久化。
