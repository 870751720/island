...
- 2026-08-28:八期头顶指示与手持工具:作业提示文字与进度圆环改为世界空间 UI,挂在玩家头顶(`src/game/ui3d/PlayerIndicator.ts`,圆环几何按进度重建,Sprite+Canvas 渲染文字,始终朝向相机);程序拼装斧子/镐子模型握在右手机器手臂末端,右下角工具按钮循环切换 空手→斧子→镐子(仅已拥有,桌面端 Q 键),默认空手,切换仅影响视觉。
