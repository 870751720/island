...
- 2026-08-28:十六期潮汐细节:①湿沙——滩面顶点色随潮位动态变化,水线以下浸湿为深沙色,退潮后以约 25 秒缓干(`IslandTerrain.updateTide`,0.25 秒节流重刷顶点色);②浪花带——新增 `world/Foam.ts`,沿水线等高线的加色混合白沫网格(96×96),涨潮时明显(透明度 0.55)、退潮时隐去(0.06),透明度平滑过渡;③浅水渐变——Ocean 构造时按地形深度烘顶点色,近岸浅绿 `#7fd0c0` 渐变到深蓝 `#4aa3c7`(smoothstep 0~1.5)。Ocean 暴露 `waterY`/`isRising` 供潮汐联动。