...
- 2026-08-28:七期作业化采集:取消交互按钮,角色停止移动且处于资源点范围内时自动作业,移动即中断;角色按资源类型播放不同作业动画(砍树双臂抡劈/凿石单臂凿/拾取弯腰扒动);每 0.6 秒命中一次并出碎屑粒子特效(`src/game/fx/Particles.ts`),树需 3 次、大石块 4 次、灌木丛 2 次、碎石/浆果 1 次,命中进度记在资源点上走开不丢;屏幕下方居中显示作业提示与进度条(工具不足时提示「需要斧子/镐子(背包中合成)」),完成时粒子爆发并入包。
