...
- 2026-08-28:八期头顶指示与手持工具:作业提示与进度指示移到玩家头顶;进度为 3D 圆环(`src/game/ui3d/PlayerIndicator.ts`,仅圆环,始终朝向相机,不可交互时完全隐藏);进度改为连续值(已命中次数 + 本次挥动进度,每帧驱动,平滑从 0 填到 100%);提示文字为 HTML UI——游戏侧将头顶位置投影为屏幕坐标,React 层用 ref 直写 DOM 渲染胶囊标签,避免每帧重渲染。程序拼装斧子/镐子模型握在右手手臂末端,右下角工具按钮循环切换 空手→斧子→镐子(仅已拥有,桌面端 Q 键),默认空手,切换仅影响视觉。
