...
- 2026-08-28:十三期河流表现重构:河流不再用一串圆形 carve+圆盘拼接(重叠处透明度叠加出深色圆斑、边缘呈扇贝状),改为独立中心线折线方案——地形 carve 用点到折线距离(等宽 RIVER_HALF_WIDTH),水面为沿折线生成的连续等宽条带网格(`buildRiverMesh`);水洼/河流/海的水材质参数统一(roughness 0.35、opacity 0.92);`isNearWater` 同步覆盖河流折线。
- 2026-08-28:十四期河流调优:河床深度固定为与水洼一致的 1.6(不再按地形高度挖深);河流改为从随机内陆点出发(与水洼保持间距,不再从水洼接出),避免与水洼衔接断开的问题。