...
- 2026-08-28:十二期水域修正:资源点生成避开所有水面及边缘 1m;水洼数量不写死,按岛屿面积(约每 3200 平方单位 1 处,3~14 处)与最小间距(max(18, 尺寸/8))约束;新增河流——全岛最多一条,从最靠岛心的水洼出发蜿蜒挖至入海,河床挖至海平面下、河面与海平面同高;水域统一为 `waterAreas`(carve+水面圆盘),喝水判定统一走 `isNearWater`。
