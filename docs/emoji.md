...
- 仿 `MilkIcon`/`PlayerNameTag` 先例:CanvasTexture + Sprite;每个表情一张共享 160×128 纹理(懒绘制缓存),材质每气泡独立以便淡出后 dispose。
- 纹理为白色药丸底(`roundRect` 全圆角 + 轻投影,同小狗表情气泡样式),emoji 按字形实际墨迹包围盒(`measureText` 的 actualBoundingBox)精确居中在药丸中心,消除各平台 emoji 字体度量差异;整体高 0.88 个世界单位(首版裸表情的 0.8 倍),宽按 1.25 比例。
- `show(target, glyph)` 以玩家根组为键,同一玩家重复发撤旧换新(天然限频);`update(delta)` 每帧跟随玩家脚部世界坐标 + Y 3.44,**气泡底边刚好贴在联机名牌顶边**(名牌中心 2.65、半高 0.35)。挂在场景层而非玩家组内,不受游泳前倾姿态影响。
...
- 气泡改为白色药丸底 + 轻投影(同小狗表情样式),整体缩小一半(高 1.1 → 0.55),位置从 Y 3.5 下移到 3.28,底边刚好贴联机名牌顶边。
- 清单去掉 ❤️ 爱与 😴 困了,保留 4 个:😍 喜爱、😭 大哭、😡 生气、😱 惊吓。

### 气泡尺寸回调(2026-09)

- 缩小一半后偏小,回调为首版裸表情的 0.8 倍(高 0.88),底边保持贴联机名牌顶边(Y 3.44)。

### 表情墨迹居中修复(2026-09)

- emoji 在白底内偏移:改用 `measureText` 墨迹包围盒(actualBoundingBox)精确居中到药丸中心,不再依赖基线经验偏移,各平台字体渲染均居中。

### 桌面端数字键直发(2026-09-12)

- 电脑版键盘 1~4 直接发送对应表情(与选择面板顶部表情区顺序一致),实现见 `docs/desktop-shortcuts.md`。

### 自绘表情图标(2026-09-13)

- 4 个表情由系统 emoji 换为自绘 SVG 图标(黏土平涂风,与道具图标同语言):喜爱(眯眼笑+漂浮爱心)、生气(怒纹+咬牙)、大哭(闭眼+泪柱)、惊吓(瞪眼+冷汗)。
- `src/game/social/EmojiIcons.ts` 为唯一图标源:每个字形一份 SVG 内部标记(viewBox 0 0 64 64)。选择面板经 `src/ui/icons/EmojiFaceIcon.tsx` 渲染;头顶气泡(`EmojiBubbles`)把同一份 SVG 栅格化进药丸纹理(构造时预载,解码完成前用系统 emoji 兜底纹理过渡),两端表现一致。
- 字形(Unicode emoji)仍作为联机线上 ID、白名单键与键盘直发映射,协议、存档、快捷键全部不变。
