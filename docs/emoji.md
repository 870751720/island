...
- 纹理为白色药丸底(`roundRect` 全圆角 + 轻投影,同小狗表情气泡样式),emoji 绘制在药丸中央;整体高 0.55 个世界单位,宽按 1.25 比例。
- `show(target, glyph)` 以玩家根组为键,同一玩家重复发撤旧换新(天然限频);`update(delta)` 每帧跟随玩家脚部世界坐标 + Y 3.28,**气泡底边刚好贴在联机名牌顶边**(名牌中心 2.65、半高 0.35)。挂在场景层而非玩家组内,不受游泳前倾姿态影响。
