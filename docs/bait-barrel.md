...
### 堆肥彩蛋:南瓜种子(2026-09-10)

- 权威端每发酵 1 个食物有 0.5% 概率桶里额外冒出 1 颗南瓜种子(`BaitBarrel.seeds`),与鱼饵一并存放,「收取」或铲回时一起到手;面板角标以 🌱×N 显示。
- `seeds` 为快照/存档可选字段(旧档缺省 0),世界增量 `fields` 随 `bait` 一并回流,协议结构不变。
