...
木箱、铁箱、养鱼池和食料桶共用储物格显示：设施区与背包区均保持 48×48px、五列和 6px 间距。底部单行名称复用 SlotItemName，短名称静止，长名称缓慢往返、两端停留；数量由 SlotItemCount 固定在右上角。空格不显示名称，食料桶的不可投喂物品继续淡显，点按整格转移及长按步进不变。
...
`feedBarrel` 使用相同的二十格库存、存取、回收与存档同步，另限制只能存入适配动物的食物，并提供走近取食接口。四级工作台配方为 3 铁锭＋3 木头＋1 冒险家的经验书。详见 [动物驯养与食料桶](animal-husbandry.md)。
