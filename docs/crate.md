...
- 2026-08-30:木箱退出工具切换循环,不再手持放置;改为背包点击「使用」原地放下(校验与工作台摆放一致,另要求与其他木箱距离 ≥0.8),与锄头挖来的丛同一套使用心智。
- 2026-08-30:手持锄头靠近木箱站定自动挖走(基础 2 下、精致 1 下):变回木箱道具,箱内物品一并回到背包(放不下掉在身旁)。
- 2026-09-05:木箱放置朝向不再随机,取玩家面向并量化为上下左右四向;朝向随存档(`rotY`)与联机世界增量同步。

- 2026-09-05:木箱配方调整为 3 木头(原 4 树枝)。
