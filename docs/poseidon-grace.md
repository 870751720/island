...
- 仅**单机模式**(非联机)生效：本机首次游玩的玩家第一次死亡必定触发，不受天数限制且优先于复活石、不消耗复活石。其他情况保持：玩家在**前 20 天**(第 1~20 天)死亡时,有 **20% 概率**触发「波塞冬的庇佑」。
- **单局仅可触发一次**(入档,读档不刷新次数)。
- 触发后:出现类似联机等待复活的倒计时界面,但视觉上明确传达「波塞冬的庇佑触发了、你被复活了」(海洋主题:三叉戟、海蓝光、专属文案)。**3 秒后**在复活落点复活。
- 复活时**不掉落任何物品**:单机死亡本就不执行随身掉落,背包/装备/工具/存档全部原样保留,生存状态回满。
- 复活后复活落点旁放置一只**赠礼木箱**,内含:二级装备一套(皮帽 `furHat`、皮衣 `furShirt`、皮裤 `furPants`、皮背包 `furBackpack`)与一封**海神的信**(`letter`)。
...
- 死亡分支先消费本机首次死亡保底资格；没有资格时尝试复活石，失败后判定
...
`island.firstDeathBlessing.v1` 独立保存 pending/used；没有标记且没有历史存档或传承数据时建立资格。死亡清档、重新开局不重置，刷新后继续 pending。旧玩家保持原概率规则；联机不消费资格。无账号条件下按浏览器本地数据识别，清除站点数据后无法识别历史游玩；存储不可用时仅在当前页面保留标记。

### 赠礼木箱(`src/game/systems/CrateSystem.ts`)

- 新增 `spawnGift(x, z, kinds)`:在指定干地直接生成预填内容的普通木箱(绕过背包与 canPlace),走统一的 `onChanged` 增量上报。
- 落点策略:复活落点周围 5 个偏移位依次找干地,全部失败则放在复活落点本身。木箱就是普通木箱,可正常存取/挖走,自动入档。
...
- 死亡界面(`src/ui/DeathScreen.tsx` 新增 `poseidon` 模式):海洋渐变背景、发光三叉戟 🔱(呼吸光晕动画)、「波塞冬的庇佑」标题与文案、蓝色倒计时「N 秒后在复活落点苏醒」。
- 场内特效:复活瞬间复活落点海蓝(`#2ec4b6`)光柱三段迸溅 + `success` 音效 + 提示「海浪把你送回了复活落点,波塞冬在身旁留下了一只木箱」。
...
- 2026-09-08:首次实现。单机前 20 天死亡 20% 概率触发,3 秒后复活落点复活,赠礼木箱(二级装备 + 海神的信),信为独立新道具且不可钓得,单局限一次并入档。
