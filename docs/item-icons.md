...
- 2026-09-04 二次迭代:
  - 追加 8 个自绘图标:皮毛、烤小鱼、果树种子、草衣、草裤、木围栏、石围栏、浆果丛(用户反馈 emoji 不贴切);
  - 级别角标改为白底黑字(浅灰描边);
  - 修复客人端无拾取飘字:客人端入包不走 `Inventory.add`,`netApplyHud` 对比快照前后数量差补发 `emitPickup`,且 `flushPickups` 不再限房主端(见 docs/multiplayer.md)。
