...
- 铁矿开采门槛提高为二级镐(石镐),自动换工具与头顶提示同步区分;铁矿密度 10→40(原数量太少,全北岛仅约 6 处)。
- 饵料桶与冶炼炉取消单次投放容量上限(原 20 个)。
- 铁矿外观改为整体锈红岩体嵌深灰金属块,与灰色石矿一眼区分(原为灰岩嵌小红斑,不易辨认)。
...
- 冶炼比例由 1:1 改为 **3 块铁矿石炼 1 块铁锭**,单炉耗时由 5 秒提高到 **15 秒**(`SmelterSystem.ts` 的 `SMELT_ORE_PER_INGOT`/`SMELT_INTERVAL`);炉内矿石不足 3 块时不冶炼、保留倒计时;面板与文档文案同步。

### 不足一炉视为空闲(2026-09)

- 修复:矿石不足 3 块时旧逻辑仍持续倒计时(走完即重置),火光长亮却永不出锭。现改为不足 3 块视为空闲:不倒计时(`tickLeft` 保持满值)、炉门火光熄灭、HUD 进度显示 0,补足矿石后自动继续。`SMELT_ORE_PER_INGOT` 常量移至 `Smelter.ts` 并由 `SmelterSystem.ts` 再导出。
