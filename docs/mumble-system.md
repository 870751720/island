...
| 条件 id | 判定 | 引导方向 | 冷却 |
|---|---|---|---|
| drinkCloser | 口渴为0、距求生饮水点≤1.5米且仍在岸上、未采集(持续2s) | 再向浅水走一点，20句 | 10s |
...
| craftSmelter | 有工作台和镐子且没有熔炉 | 建熔炉 | 900s |
...
- **条件冷却**:上表;边沿型天然不重复。
- **全局间隔**:普通台词之间至少20秒（开局第一句也在20秒左右出现）；缺水接近提醒 `drinkCloser` 可在上一句之后至少10秒触发，自身另有10秒冷却，说完重置全局计时。冷却期与单条件冷却期间边沿状态仍持续刷新,短事件(如陨石)不会被漏检。
