...
- **站定自动安放**:手持站定不动累计进度(统一 2 秒),走满自动放置,头顶圆环走进度;移动即复位,普通设施一次停止移动只放一个;切其他工具或道具耗尽即结束安放。食物种子与树种站定即播，不等待 2 秒，成功后紧接下一格，不需移动重新触发。
...
食物种子与三种树种注册 `holdTime: 0`、`repeatOnSuccess: true`，每次权威更新最多结算一颗，成功后下一帧重新寻找面前 3×3 范围内的最近有效格，无额外等待。普通设施保留原有等待时长与单次站定限制。移动、游泳、其他忙碌动作、无有效格或种子耗尽时停止；结算失败后等待移动重试，避免重复提示。切换种类会清空旧进度，切入种子解除此前设施留下的单次限制。图鉴的种子属性与开垦指南同步说明该机制。
