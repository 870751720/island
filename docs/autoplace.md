...
资源占位以生命周期判定：树桩、岩石、铁矿、碎石堆和陨石耗尽后释放位置，存档中保留的隐藏记录不阻挡放置；树苗和等待再生的草丛、灌木、浆果丛、蚯蚓窝继续占位，整株挖走后释放。预览、设施结算、围栏及播种共用该规则。

手机摇杆微调设施落点时，手持背包设施自动获得「稳稳安放」状态，移动速度降低 40%，方便对准网格。收起或耗尽后恢复；仍有余量且继续手持时保持生效。覆盖安放、围栏、围栏门及树木种子等注册道具，不包含锄头开土；红色不可放预览也保持减速。详见 [Buff 系统](buff-system.md)。
...
- **点「使用」手持安放**:背包里点「使用」进入手持模式,显示面前附近最近可放网格的预览,站定 2 秒后放下;三种树木种子也归为设施，树苗按整数网格整齐种植；种子采用下述连续播种规则。
- **长按工具按钮选择切入**:长按工具按钮按住不动约 0.35s 弹出(提前抬起算普通点击,长按触发前手指移动超过 12px 取消本次操作)手持选择面板(`src/ui/PlacePicker.tsx`),平铺展示所有可切换的手持项:空手、已拥有的普通工具(斧/镐/铲/鱼竿/弓/剑/套索)与背包里所有可手持放置的道具(含木围栏/石围栏/围栏门,图标+名称+剩余个数角标),当前手持高亮,可放置道具中上次使用的排最前;点选直接切入对应手持模式(工具走 `Game.selectTool`,道具走 `Game.pickPlaceItem`,均经 `tool` 动作同步)。单击循环仍含全部可放置道具(空手 → 工具 → … → 每种可放置道具各一格),长按面板是快速直达入口。手持时手里举着该物体的真实建模(整体缩到最大边约 0.3),面前落点常驻半透明幽灵预览,**绿=可放、红=不可放**,附近一圈全放不下时头顶显示不可放原因(与「需要斧子」同款提示)。
- **站定自动安放**:手持站定不动累计进度(统一 2 秒),走满自动放置,头顶圆环走进度;移动即复位,普通设施一次停止移动只放一个;切其他工具或道具耗尽即结束安放。食物种子与树种每颗播种耗时 0.5 秒，成功后紧接下一格，不需移动重新触发。
- **手持时占住双手**:手持设施(安放/围栏/围栏门)期间,除安放以外的所有空手站定交互(采集/摘果/挤奶/喝水等)一律让位不可进行(`Game.isSessionBusy` 统一拦截,与牵羊占手同款约定;房主/客人两端同规则),切回其他工具即恢复。
...
- **UI**:背包可放置道具「使用」统一路由到 `Game.pickPlaceItem(kind)`(含树木与作物种子,漂流瓶/信走各自入口);工具按钮长按选择面板与角标照旧,数据源不变。
- **联机**:手持选择经 `tool` 动作上行,房主推进站定安放并权威结算;客人端 `updatePreviewFor` 本地驱动预览。详见 `multiplayer.md`。
...
- 2026-09-09(迭代):可放置道具(约 30 种)串在工具循环链上切换繁琐,新增**长按工具按钮弹出手持选择面板**作为快速直达入口(按住不动约 0.35s 触发,提前抬起算普通点击,长按触发前手指移动超过 12px 取消本次操作);面板含空手、已拥有工具与全部可放置道具(当前手持高亮,上次使用的道具排最前),单击循环行为保持不变;面板含空手、已拥有工具与全部可放置道具(当前手持高亮,上次使用的道具排最前)。`Game.pickPlaceItem(kind)` 为道具统一入口,选中仍走 `selectTool`/`tool` 动作,同步协议不变;HUD 快照新增 `placeables` 字段。
...
### 2026-09-09(迭代):修复循环在多个可放置道具间乒乓

- 问题:循环列表按「上次使用的道具排最前」动态重排,而循环选中本身会更新"上次使用"——选中下一项后该项跳到最前,当前项位置随之改变,多个可放置道具(如胡萝卜种子/小麦种子)间来回乒乓,永远切不回空手与其他工具。
- 修复:`Game.cycleTool` 构造循环列表时不再传 `lastPlaceKind`,改用背包固定顺序,循环序列稳定;「上次使用的排最前」仍保留给长按手持选择面板(`placeableList`)。

- 2026-09-11(迭代):**制作完成自动手持收窄 + 背包「使用」改为进入手持安放模式**。制作完成后自动拿在手上的设施仅限火把/火堆/一级工作台三种(其余设施产物进背包,需要时从背包或选择面板手持);原「手持铲子时不自动切换」规则删除。背包「使用」可放置道具不再就近最优格瞬间直放,改为与选择面板一致的 `pickPlaceItem` 切入手持模式(幽灵预览 + 站定自动放置),`Game.useFacilityItem` 与联机 `useFacility` 动作删除,`settleFacility` 只保留站定放置的 cell 路径。协议动作集变化,`NET_PROTOCOL_VERSION` 24→25;存档不变。
- 2026-09-11(迭代):**手持设施期间屏蔽其余空手交互**。新增 `Player.holdsFacility` 判定(安放/围栏/围栏门),`Game.isSessionBusy` 对非安放交互统一返回占用,采集/摘果/挤奶/喝水等站定空手交互让位,只有安放可进行;作物采收原本就要求空手,自然覆盖。存档与协议不变。
- 2026-09-11(迭代):**工具按钮显示条件补全**。显示条件补上锄头(`hasHoe`,原列表漏了锄头,做了锄头按钮却不出现);并新增「背包里有任何可放置设施道具(`placeables` 非空,含围栏/围栏门/箱子/火堆等全部注册设施)时按钮也显示」,空手但有设施在包也能直接手持安放。HUD 快照新增 `hasHoe` 字段。
- 2026-09-13(迭代):**选择面板点按改为 click 结算并吞掉长按尾 click**。手持项/表情点选与点面板外关闭统一在 `click` 上结算(原来是 `pointerdown`):按下即选中会立刻卸载面板,手指还没抬起,浏览器随后派发的 click 命中面板下方露出的按钮(面板与背包按钮同处右侧,重叠区域点击会同时触发选择与误开背包)。click 是一次点按的终结事件,结算后卸载不会再有后续 click 重定向,误触与面板内滚动误选(滚动产生 `pointercancel`,不触发 click)一并解决。另:长按弹出面板的那次按住抬起后仍会派发一次 click,会命中刚出现的遮罩把面板立即关掉——面板挂载后监听第一个抬起(期间无新按下即属于开面板那次按住),在捕获阶段吞掉紧随的那一次 click。

## 连续播种

食物种子与三种树种注册 `holdTime: 0.5`、`repeatOnSuccess: true`，每颗累计 0.5 秒后结算，成功后重新寻找面前 3×3 范围内的最近有效格并开始下一颗进度，两颗之间无额外等待。普通设施保留原有等待时长与单次站定限制。移动、游泳、其他忙碌动作、无有效格或种子耗尽时停止；结算失败后等待移动重试，避免重复提示。切换种类会清空旧进度，切入种子解除此前设施留下的单次限制。图鉴的种子属性与开垦指南同步说明该机制。

## 成功放置反馈

所有背包设施成功落地在统一结算入口播放 1 号轻柔落土声，种子通过注册项选择更短、更轻的播种声。失败不播，原有设施内部成功声由此替代；锄头开土保持原反馈。连续播种声音按玩家合并过密事件，播种本身不增加等待。详见 [音乐与音效](audio.md)。
