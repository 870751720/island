...
- 2026-09-13(迭代):**选择面板改为 pointerup 触发**。手持项/表情点选与点面板外关闭从 `pointerdown` 改到 `pointerup`:此前按下即选中并卸载面板,手指尚未抬起,浏览器随后派发的 click 会重定向到面板下方露出的按钮(面板与背包按钮同处右侧,重叠区域点击会误开背包);改为抬起再结算后整个点按期间面板保持存在,click 不会再落到别的按钮。顺带修复在面板内滚动时一按下就误选手持项的问题(滚动产生 `pointercancel`,不再触发点选)。
