...
- 种植作物接入烤/煮产线:胡萝卜可烤成**烤胡萝卜** cookedCarrot(12/1/2,🍢)、可煮成**胡萝卜汤** boiledCarrot(10/5/3,🍲);小麦先在磨坊按 1:2 磨成面粉，面粉可烤成**烤面包** bread(15/0/3,🍞)，每份面粉产出一份面包；小麦保留 burnTime 15 的燃料用途(见 mill.md)。三者均可入饵料桶(胡萝卜制品 4 饵/个、面包 2 饵/个)。
...
- 煮汤「收取」改为状态返回 `'ok' | 'empty' | 'full'`:背包放不下汤品(判 `canFit`,已有同种格可无限叠加)时不消耗台面产物并提示「背包满了」,不再掉地上;火堆批量烤制在发起时判一次 `canFit`,放不下烤制品则不允许开烤并提示,烹饪中途被塞满则暂停发放(不掉地),腾出空间自动继续。
