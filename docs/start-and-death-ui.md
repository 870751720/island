...
## 需求描述
- 使用「海岛手记」视觉：纯代码海绿色渐变背景、奶油色标题、雾蓝色主按钮与罗盘装饰，不展示小岛模型。
...
- `styles.ts` 定义响应式视觉。约 375px 手机将罗盘、标题和操作卡片居中纵向排列，以留白控制节奏；电脑宽屏和低高度横屏均采用左侧标题、右侧操作卡片的两列布局，不保留岛屿占位。短屏可纵向滚动，安全区留白，所有按钮触控高度至少 44px。
- `start/palette.ts` 共享纯 CSS 渐变底色，开始页增加轻微呼吸的椭圆波纹与光晕，不使用背景图片或生图资源；电脑和手机均由 CSS 自适应绘制。
- 标题、入口与罗盘图标独立排版，操作区使用细边框、半透明渐变与柔和阴影组织层次；背景直接由 CSS 绘制。减少动态效果的系统偏好会关闭 CSS 动画。
...
### 背景与声音
- 开始页仅使用 CSS 背景与 SVG 图标，不创建 WebGL 渲染器，不加载装饰模型或运行 3D 动画循环。
