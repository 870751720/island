...
详见 `autoplace.md`。可安放道具(建筑/丛/蚯蚓窝/神龛/熄灭的火堆等)统一改为「限定位置 + 自动安放」:新增动作 `useDeadCampfire`(熄灭火堆道具放置);手持安放工具(`place`,随客人 `tool` 动作上行)与各物品放置动作仍走既有 `useCrate/useShrine/useBush/...` 上行、房主权威结算,落点由房主按该客人就近最优可放格中心计算。落点预览(绿/红+原因)与站定进度在两端各自本地驱动,不新增快照字段、不改存档版本。
