...
- 四个新动作:`lassoThrow [dx, dz]`(掷出瞬间房主权威扣一个套索、补甩索动作窗口、广播 `lassoThrown` 视觉事件,仿 `arrowShot`)、`lassoHit [animalId, x, z]`(客人本地扫掠判定套中后上行,房主 `Wildlife.lassoSheep` 权威置拴绳状态,仿 `arrowHit`)、`lassoMiss [x, z]`(掷空:套索掉在落点,经掉落物世界段回流)、`lassoStake / lassoUntie`(脚下打桩 / 解开身旁被拴的羊,套索回收经 HUD 快照回流)。
...
- 拴羊桩为新增世界段 `stakes`(落点 + 稳定 id),打桩/拆桩走世界增量;拴羊 AI(牵引跟随/绕桩吃草/绷断滑脱)只在房主端结算,客人端只插值姿态。
- 断线兜底:客人离场时其牵着的羊由房主释放、套索掉在羊脚下;被拴在桩上的羊不受影响。存档时被牵着(未打桩)的羊退回套索并释放(联机欢迎包快照不改现场状态);拴住的羊随 `stakes` 入档,读档在权威端于桩位重新生成。
