...
开始页采用独立的日落海天背景、程序化动态小岛与本地菜单声音。创建房间、加入房间仍经 `StartScreen.onMultiplayer` 进入原有 `RoomLobby`，房主权威结算、客人动作上行与快照回流机制不变。菜单场景不创建游戏世界，其动画及音频不走网络同步；进入任一大厅时销毁菜单渲染器和音频上下文。
