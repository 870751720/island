...
详见 `autoplace.md`。可安放道具(建筑/丛/蚯蚓窝/神龛/熄灭的火堆等)统一改为「限定位置 + 自动安放」:新增动作 `autoPlaceHold [kind|null]`(客人进入/退出手持安放模式,仅表现状态)与 `useDeadCampfire`(熄灭火堆道具放置);各物品的放置动作仍走既有 `useCrate/useShrine/useBush/...` 上行、房主权威结算,落点由房主按该客人面前吸附格中心计算。落点预览(绿/红)与站定进度在两端各自本地驱动,不新增快照字段、不改存档版本。
