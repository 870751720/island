...
- 原版男孩的肩肘、膝踝步态和收颌头型由各端按既有性别、GM 造型及实际移动速度生成。房主与客人共用实现，不增加网络字段；动画不改变权威位移、碰撞或命中，步态相位允许各端不同。女孩维持原表现，存档版本不变。

- 原版男孩的露颈肩线和微分碎盖由各端模型工厂生成，沿用性别与 GM 造型同步，不新增网络或存档字段；头部高度变化仅影响本地模型表现和既有头部挂点。
