...
房主、客人和远程玩家均由 `Player` 使用 `BoyModel.ts` 创建黏土小男孩。肩部、髋部关节由现有移动和动作姿态驱动；模型颜色与受击材质按玩家独立创建。基础服装没有装备属性，装备状态仍走原同步流程。本外观不增加网络字段或权威结算，动作上行、快照回流及事件补播规则不变，详见 `player-model.md`。

### 玩家装备穿搭

12 件穿戴装备使用 `BoyWardrobe` 与 `OutfitModels` 在房主、客人及远程角色上统一构建。穿戴/卸下沿用 `equipItem` / `unequipItem` 上行动作，由房主权威结算；原有装备快照回流后调用 `Player.setEquip`，重复 ID 不重建，变更时仅替换对应槽位模型。初次加入、重连及读档直接按现有装备 ID 恢复新造型。袖口、肩片、裤袋、护膝挂载原有动画关节，随玩家姿态在各端本地表现，无新增事件补播、协议字段或存档版本变化；属性和容量规则保持原样。
