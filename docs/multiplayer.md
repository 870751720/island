...
开始页采用纯 CSS 海绿色渐变背景、程序化动态小岛与本地菜单声音。创建房间、加入房间仍经 `StartScreen.onMultiplayer` 进入原有 `RoomLobby`，房主权威结算、客人动作上行与快照回流机制不变。菜单场景不创建游戏世界，其动画及音频不走网络同步；进入任一大厅时销毁菜单渲染器和音频上下文。

大厅沿用海岛手记视觉，创建与加入页面共享响应式卡片、邀请口令、队友列表和连接提示样式；形象设置复用岛民档案弹窗。上述显示状态仍取自现有 NetHost / NetGuest 与个人档案，房主权威结算、客人连接流程和协议保持兼容。

### 菜单营地表现
开始页营地复用游戏树木与一级帐篷模型，仅运行本地风摇、浪纹与营火动画；不创建房主或客人世界状态。开始页、形象设置与联机大厅主操作统一使用雾蓝配色，不改变创建、加入、恢复存档和开局协议。
