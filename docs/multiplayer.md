...
配乐属于客户端表现:自动选曲使用已同步的季节(含 GM 季节覆盖)、本人 FishingSystem 状态及房主同步的雨雪强度/未包络风量。客人钓鱼仍由动作上行、房主结算、状态回流后驱动配乐,队友动作不切换本机音乐。GM 音乐选曲只在发起者本机锁定,不广播、不持久化;无需新增网络协议或存档字段。

天气配乐在客户端按强度回差选择(0.55 进入、0.35 退出),优先级为本人钓鱼、天气、季节;客人不自行随机天气。GM 立即选曲只断开本机旧曲音符输出,不影响环境声、玩法音效及其他玩家。
