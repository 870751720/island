...
export function candidate(index: number): CompanionModel {
  ...
  if(style.fluff) {
    for(const side of [-1,1])
      ellipsoid(body,white,[side*.22,.83,.32],[.19,.18,.17],6);
  }
  ...
  for(const side of [-1,1]) {
    ...
    if(style.fluff) for(let i=0;i<2;i++) {
      const tuft=ellipsoid(head,white,[side*(hx*.77+i*.05),-hy*.16-i*.10,.035],[.16-i*.02,.13,.17],5);
