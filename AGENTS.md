...
- 主分支为 `main`。改完代码后先执行下述「部署前本地验证」流程，用户明确确认本地验证通过后，才提交并通过 `npm run deploy` push 到 `origin main`，触发 GitHub Actions 部署到 GitHub Pages。
...
- **标准部署命令**：完成类型检查/构建并提交后，在仓库根目录执行 `npm run deploy`。脚本检查干净的 `main` 工作区，读取 `githubtoken.txt`，推送本次 SHA，按 SHA 等待 `deploy.yml` 的 Actions 成功，再用 curl 验证线上 HTTP 200。后续会话统一使用此命令，不再临时拼接推送与轮询脚本。需 Node 22.18+、Git 和 curl；详见 `docs/deployment.md`。
...
- 本地包交付时提供 ZIP 链接、实际大小、解压目录及入口文件的绝对路径链接，说明本次改动，交由用户本地运行验证。**必须等待用户明确确认本次验证通过，才能提交、push 或执行 `npm run deploy`；构建成功、解压成功或用户未回复都不代表验证通过。** 用户反馈问题后继续修复、重新打包解压并交付验证。
- 用户确认后执行 `npm run check` 检查正式站点构建，提交并执行 `npm run deploy`，等待 Actions 成功与线上 HTTP 200 后才报告部署成功。
- **测试分工**：ZCode 负责类型检查、构建、ZIP 校验及解压，不起 dev 服务器、不做浏览器冒烟测试；用户负责本地运行时验证。完整流程：改码 → 类型检查 → 本地 H5 打包、校验与解压 → 用户验证并明确确认 → 正式构建检查 → 提交 → 部署并等待验收 → 交付线上链接。
